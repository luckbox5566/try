...
public class AdultPainActivity extends AppCompatActivity implements View.OnClickListener {
    @BindView(R.id.iv_colse_adult)
    ImageView ivColseAdult;
    @BindView(R.id.tv_commit_adult)
    TextView tvCommitAdult;
    @BindView(R.id.tv_title_adult)
    TextView tvTitleAdult;
    @BindView(R.id.tv_pinggu_adult)
    TextView tvPingguAdult;
    @BindView(R.id.tv_score_adult)
    TextView tvScoreAdult;
    @BindView(R.id.fen)
    TextView fen;
    @BindView(R.id.rc_adult)
    RecyclerView rcAdult;
    @BindView(R.id.gv_adult)
    CoustomGridView gvAdult;

    private String result = Constant.ADULTPAIN;
...
//    private RcyAdapter adapter_rec;
    ...
    private void initCanShu() {
        ...
        if (tag.equals("add")) {
            ...
            tvPingguAdult.setText(fenxian);
            tvScoreAdult.setText(updataFen);
...
//        loginid = instance.getYhgh();//用户工号，登录ID 1000
    ...
    private void initData() {
        ...
        LinearLayoutManager manager = new LinearLayoutManager(AdultPainActivity.this);
        rcAdult.setLayoutManager(manager);
//        adapter_rec = new RcyAdapter(AdultPainActivity.this, da.json(result), inPfxx, tag, "adultpain");
//        rcAdult.setAdapter(adapter_rec);

        listCb = new ArrayList<>();
        for (int i = 0; i < endNames.length; i++) {
            listCb.add(new CheckBoxBean(endNames[i], beanCsxx[i],"0"));
        }
        cbadapter = new MoreChooseAdapter(AdultPainActivity.this, listCb, tag, R.layout.gv_item_checkbox);
        gvAdult.setAdapter(cbadapter);
    }

    private void initListener() {
        ivColseAdult.setOnClickListener(this);
        tvCommitAdult.setOnClickListener(this);
        callRcy();
...
//        });
    ...
    private void total() {
        if (all == 0) {
            tvPingguAdult.setText("无痛");
        } else if (all >= 1 && all <= 3) {
            tvPingguAdult.setText("轻度疼痛");
        } else if (all >= 4 && all <= 6) {
            tvPingguAdult.setText("中度疼痛");
        } else if (all >= 7 && all <= 10) {
            tvPingguAdult.setText("重度疼痛");
        }
        String fxjb = tvPingguAdult.getText().toString().trim();
        getCurDate(fxjb);//提醒日期
        Log.e("提醒日期", date);

        tvScoreAdult.setText(all + "");
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iv_colse_adult:
                finish();
                break;
            case R.id.tv_commit_adult:
                if (tag.equals("add")) {
                    ...
                    if (num < 6) {
                        ...
                        centerDialog = new CenterDialog(AdultPainActivity.this,R.layout.dialog_commit,new int[]{R.id.bt_yes,R.id.bt_no});
                        centerDialog.setOnCenterItemClickListener(new CenterDialog.OnCenterItemClickListener() {
                            @Override
                            public void OnCenterItemClick(CenterDialog dialog, View view) {
                                if(view.getId()==R.id.bt_yes){
                                ToastUtils.showLoading(AdultPainActivity.this);
                                netWorkAdd();
                                netWorkTime();}
                            }
                        });
    ...
    private void netWorkUpdata() {
        ...
        String allscore = tvScoreAdult.getText().toString().trim();
        String fenxian = tvPingguAdult.getText().toString().trim();
        String curdate = dateUtil.getDate();
        String canshu = brzyid + f + curdate + f + allscore + f + fenxian + f + allString + f + pingguren + f + curdate + f + "7" + f + "" + f + ""
    ...
    private void netWorkAdd() {
        ...
        String fenxian = tvPingguAdult.getText().toString().trim();
        String curdate = dateUtil.getDate();
        String canshu = brzyid + f + curdate + f + allscore + f + fenxian + f + allString + f + pingguren + f + curdate + f + "7" + f + "" + f + sqsh
        ...
        zhierCall.start(new NetWork.SocketResult() {
            ...
            @Override
            public void fail(String info) {
                ToastUtils.makeToast(MyApplication.getContext(), "请求数据失败");
            }
        });
    }
...
//        示例：20111201¤0404¤压疮¤2017/6/14 15:33:24
        String canshu = brzyid + f + bqId + f + "疼痛成人" + f + date;
        //网络参数设置
        Log.e("添加时间提醒", canshu);
        zhierCall = (new ZhierCall())
                .setId(uesrid)
